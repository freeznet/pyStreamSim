...
if __name__ == '__main__':
    ...
    for spd in initSpeed:
        ...
        sid = sid + 1
    serverList.resort()
    b = Block(nowBlock, serverList, 1, playTime)
    ...
    while allStartTime<maxTime:
        ...
        if(myBlock != None and myBlock.isDone!=True):
            ...
            if(myBlock.isDone==True):
                ...
                ######### do not reach limit
                if(allStartBuffer > qlimit and myBlock.rate == newSelect and newSelect == len(rateList)-1):
                    diff = allStartBuffer - sleepTime
                    allStartTime = allStartTime + diff
                    allStartBuffer = allStartBuffer - diff
                #########

                b = Block(nowBlock, serverList, newSelect, playTime)
                b.startTime = allStartTime
                b.startBuffer = allStartBuffer
                blockList.append(b)
